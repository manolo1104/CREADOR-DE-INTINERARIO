    ...
    <>
        ...
        <div className="flex flex-col flex-1 p-5">
          ...
          <div className="mt-auto pt-4 border-t border-white/8">
            ...
            <div className="flex gap-2" onClick={noPropagar}>
              ...
              >
                {getBooking(locale).catalogo.reservar}
              </Link>
              <BotonAgregarTour
                tourId={tour.id}
                tourSlug={tour.slug}
      ...
      {abierto && (
        <div
          role="dialog"
          aria-modal="true"
          aria-label={t.informacionDe(tour.nombre)}
          onClick={cerrar}
          ...
          >
            ...
            <div className="p-5 sm:p-6 space-y-5">
              ...
              <div className="flex flex-wrap gap-x-5 gap-y-2 text-[11px] font-dm text-crema/55">
                <span className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5 text-verde-vivo" aria-hidden="true" />{tourDurTexto(tour, t.unidadHoras)}</span>
                <span className="flex items-center gap-1.5"><Users className="w-3.5 h-3.5 text-verde-vivo" aria-hidden="true" />
                  {t.rangoPersonas(tour.groupMin, tour.groupMax)}
                </span>
                <span className="flex items-center gap-1.5">
                  <span className="w-1.5 h-1.5 rounded-full bg-verde-vivo" aria-hidden="true" />
                  {t.dificultad[tour.dificultad] ?? tour.dificultad}
                </span>
              </div>

              {tour.destinos?.length > 0 && (
                <div>
                  <p className="text-[10px] tracking-[2px] uppercase text-crema/40 font-dm mb-2.5">{t.queSeVisita}</p>
              ...
              <div>
                <p className="text-[10px] tracking-[2px] uppercase text-crema/40 font-dm mb-2.5">{t.incluye}</p>
