...
export default function CotizacionesClient({ initialQuotes }: { initialQuotes: TourQuote[] }) {
  ...
  const totalAmt = lines.reduce((s, l) => s + calcLine(l), 0);
  ...
  function updateLine(i: number, field: keyof LineItem, val: string | number) {
    setLines(ls => ls.map((l, idx) => {
      ...
      if (field === "tourSlug") {
        ...
        updated.tourName = t?.nombre || "";
      }
      updated.subtotal = calcLine(updated);
      return updated;
    }));
  }
  ...
  async function saveQuote() {
    if (!form.customerName || !form.customerEmail || lines.some(l => !l.tourSlug || !l.tourDate)) return;
    setSaving(true);
    ...
    const r = await fetch("/api/admin/cotizaciones", {
      ...
      body: JSON.stringify({
        tourName: lines.map(l => l.tourName).join(" + "),
        tourSlug: lines[0].tourSlug,
  ...
  async function sendEmail(id: string) {
    ...
    if (r.ok) {
      ...
      setMsg("✅ Email enviado a tu correo admin (reenvía al cliente hasta verificar dominio)");
  ...
  function downloadPDF(q: TourQuote) {
    ...
    const heroUrl = (() => {
      const t = TOURS_DB.find(t => t.slug === q.tourSlug);
    ...
    const itemsRows = items.map(it => `
      <tr style="border-bottom:1px solid #e8e0d0">
        <td style="padding:10px 0;color:#1a2e1a;font-size:14px">${it.tourName}</td>
        <td style="padding:10px 8px;color:#8a7a5a;font-size:13px;text-align:center">${fDateL(it.tourDate)}</td>
        <td style="padding:10px 8px;color:#8a7a5a;font-size:13px;text-align:center">${it.adults}A${it.children > 0 ? ` · ${it.children}N` : ""}</td>
        <td style="padding:10px 0;color:#c4882a;font-size:14px;font-weight:600;text-align:right">$${calcLine(it).toLocaleString("es-MX")} MXN</td>
      </tr>`).join("");
...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  .cot-box{text-align:right}
  .cot-label{font-size:10px;letter-spacing:2px;text-transform:uppercase;color:#8a7a5a;font-family:Arial;margin-bottom:4px}
  .cot-num{font-size:20px;color:#1a2e1a}
  .cot-valid{font-size:11px;color:#9a8a6a;font-family:Arial;margin-top:3px}
  ...
  .tours-table thead tr{background:#f4edd8;font-family:Arial;font-size:10px;letter-spacing:1.5px;text-transform:uppercase;color:#8a7a5a}
  .tours-table thead th{padding:10px 0;font-weight:400}
  .tours-table thead th:last-child{text-align:right}
  .tours-table thead th:nth-child(2),.tours-table thead th:nth-child(3){text-align:center}
  ...
  .includes{border-left:3px solid #c4882a;padding-left:14px;margin-bottom:20px}
  .includes p{font-family:Arial;font-size:12px;color:#3a3a2e;line-height:2.1}
  .notes-box{background:#f4edd8;border:1px solid #d4ccbc;padding:12px 14px;margin-bottom:20px;font-family:Arial;font-size:13px;color:#3a3a2e}
...
<div class="header">
  ...
  <div class="cot-box">
    ...
    <div class="cot-num">${q.quoteNumber}</div>
    <div class="cot-valid">Válida 48 horas · ${new Date().toLocaleDateString("es-MX")}</div>
  </div>
</div>
...
<div class="info-grid">
  ...
  <div class="info-cell"><div class="info-cl">Email</div><div class="info-cv" style="font-size:13px">${q.customerEmail}</div></div>
  <div class="info-cell"><div class="info-cl">Teléfono</div><div class="info-cv">${q.customerPhone || "—"}</div></div>
  <div class="info-cell"><div class="info-cl">Fecha de cotización</div><div class="info-cv" style="font-size:13px">${new Date().toLocaleDateString("es-MX")}</div></div>
</div>

${heroUrl ? `<img src="${heroUrl}" alt="${q.tourName}" class="hero-img" />` : ""}

<div class="section-label">Tours incluidos</div>
...
<div class="total-row">
  ...
  <span class="total-value">${fmx(q.totalAmount)}</span>
</div>
...
${q.notes ? `<div class="notes-box"><strong>Notas:</strong> ${q.notes}</div>` : ""}
...
<div class="footer">
  ...
  <em>Cotización válida por 48 horas. Precios en pesos mexicanos (MXN).</em>
</div>
</body></html>`);
    win.document.close();
  ...
  function waMsg(q: TourQuote) {
    ...
    return `https://wa.me/${ph}?text=${encodeURIComponent(
      `Hola ${q.customerName}, aquí tu cotización *${q.quoteNumber}*:\n\n*${q.tourName}*\n\nTotal: *${fmx(q.totalAmount)}*\n\nIncluyendo transporte, guía, entradas y desayuno. Válida 48 horas.\n\n¿Confirmamos?`
    )}`;
  }
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
                ...
                return (
                  <tr key={q.id} className="border-b border-[#1a2e1a]/6 hover:bg-[#f4edd8]/50 transition-colors">
                    ...
                    <td className="py-3 px-4">
                      <p className="text-[#1a2e1a] font-medium">{q.customerName}</p>
                      <p className="text-[#1a2e1a]/40 text-xs">{q.customerEmail}</p>
                    </td>
                    <td className="py-3 px-4 text-[#1a2e1a]/70 max-w-[200px]">
                      <p className="truncate">{q.tourName}</p>
                      <p className="text-xs text-[#1a2e1a]/40">{fDate(q.tourDate)}</p>
                    </td>
                    ...
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-2">
                        <button onClick={() => sendEmail(q.id)} disabled={sending === q.id} title="Enviar email al admin"
      ...
      {/* Modal nueva cotización — multi-tour */}
      {modal && (
          ...
          <div className="relative bg-white border border-[#1a2e1a]/10 w-full max-w-2xl p-6 overflow-y-auto max-h-[95vh] shadow-xl rounded-sm">
            ...
            <div className="bg-[#f4edd8]/50 border border-[#1a2e1a]/8 p-4 rounded-sm mb-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {[
                  { l: "Nombre *", k: "customerName", t: "text", p: "Nombre completo" },
                  ...
                  return (
                    <div key={i} className="border border-[#1a2e1a]/10 p-3 rounded-sm bg-white">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        {lines.length > 1 && (
                          <button onClick={() => removeLine(i)} className="text-[#1a2e1a]/30 hover:text-red-500 transition-colors">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        ...
                        {t && <div className="sm:col-span-2 text-right text-xs font-dm text-[#c4882a]">
                          Subtotal: {fmx(calcLine(line))}
                        </div>}
                      </div>
                    </div>
                  );
            ...
            {totalAmt > 0 && (
              <div className="border border-[#c4882a]/30 bg-[#c4882a]/8 px-4 py-3 flex justify-between items-center rounded-sm mt-3">
                <span className="text-[#1a2e1a]/60 font-dm text-sm">Total cotización ({lines.length} tour{lines.length > 1 ? "s" : ""})</span>
                <span className="font-cormorant text-[#c4882a] text-xl">{fmx(totalAmt)}</span>
              </div>
            )}

            <button onClick={saveQuote}
              disabled={saving || !form.customerName || !form.customerEmail || lines.some(l => !l.tourSlug || !l.tourDate)}
              className="w-full bg-[#3a6b1a] hover:bg-[#5a9e2a] text-white py-3 text-[11px] tracking-[2px] uppercase font-dm transition-colors disabled:opacity-40 rounded-sm mt-3">
