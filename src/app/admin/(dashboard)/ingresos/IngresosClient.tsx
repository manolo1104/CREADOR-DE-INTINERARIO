...
function KpiCard({ label, value, sub, delta }: { label: string; value: string; sub?: string; delta?: number }) {
  return (
    <div className="bg-white border border-[#1a2e1a]/10 rounded-sm p-5">
      <p className="text-[10px] tracking-[2px] uppercase text-[#1a2e1a]/40 font-dm mb-2">{label}</p>
      <p className="font-cormorant text-[#c4882a] text-3xl font-light leading-none mb-1">{value}</p>
      {sub && <p className="text-[#1a2e1a]/40 font-dm text-xs">{sub}</p>}
      {delta !== undefined && (
...
export default function IngresosClient({ kpis }: { kpis: KPIs }) {
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <h1 className="font-cormorant text-[#1a2e1a] text-2xl font-light mb-6">Ingresos & Métricas</h1>
      ...
      {/* Chart */}
      <div className="bg-white border border-[#1a2e1a]/10 rounded-sm p-5 mb-8">
        <p className="text-[10px] tracking-[2px] uppercase text-[#1a2e1a]/40 font-dm mb-4">Ingresos · Últimos 12 meses</p>
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={kpis.porMes} margin={{ top:4, right:8, left:0, bottom:0 }}>
            ...
            <XAxis dataKey="mes" tick={{ fill:"#1a2e1a", opacity:0.4, fontSize:10 }} axisLine={false} tickLine={false} />
            <YAxis tickFormatter={v => fmx(Number(v))} tick={{ fill:"#1a2e1a", opacity:0.4, fontSize:10 }} axisLine={false} tickLine={false} />
            <Tooltip
              contentStyle={{ background:"#fff", border:"1px solid rgba(26,46,26,0.1)", borderRadius:2 }}
              labelStyle={{ color:"#1a2e1a", fontSize:11 }}
      ...
      {/* Top tours */}
      <div className="bg-white border border-[#1a2e1a]/10 rounded-sm p-5">
        <p className="text-[10px] tracking-[2px] uppercase text-[#1a2e1a]/40 font-dm mb-4">Tours más vendidos</p>
        {kpis.toursMasVendidos.length === 0
          ? <p className="text-[#1a2e1a]/30 font-dm text-sm">Sin datos aún</p>
          : <table className="w-full font-dm text-sm">
              <thead>
                <tr className="border-b border-[#1a2e1a]/10 text-[#1a2e1a]/40 text-[10px] uppercase tracking-[1px]">
              ...
              <tbody>
                {kpis.toursMasVendidos.map(t => (
                  <tr key={t.nombre} className="border-b border-[#1a2e1a]/6">
                    <td className="py-2.5 pr-4 text-[#1a2e1a]/80">{t.nombre}</td>
                    <td className="py-2.5 pr-4 text-center text-[#1a2e1a]/60">{t.count}</td>
                    <td className="py-2.5 text-right text-[#c4882a] font-medium">${t.ingresos.toLocaleString("es-MX")}</td>
                  </tr>
                ))}
              </tbody>
            </table>
