    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[#1B4332]/50 font-dm text-sm mt-1">{clientes.length} clientes únicos</p>
        </div>
      </div>
      ...
      <div className="bg-white border border-[#1B4332]/10 rounded-sm overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm font-dm">
            ...
            <tbody>
              {filtered.length===0 && <tr><td colSpan={5} className="py-12 text-center text-[#1B4332]/30 font-dm">Sin clientes</td></tr>}
              {filtered.map(c => (
                <tr key={c.email} onClick={() => setSelected(c)}
                  ...
                  <td className="py-3 px-4">
                    ...
                    <p className="text-[#1B4332]/40 text-xs">{c.email}</p>
                  </td>
                  <td className="py-3 px-4 text-center text-[#1B4332]/70">{c.totalReservas}</td>
                  <td className="py-3 px-4 text-right text-[#52B788] font-medium">{fmx(c.totalGastado)}</td>
          ...
          <aside className="relative w-full max-w-sm bg-white border-l border-[#1B4332]/10 flex flex-col overflow-y-auto shadow-xl">
            <div className="flex items-start justify-between p-5 border-b border-[#1B4332]/10">
              <div>
                ...
                <p className="text-[#1B4332]/40 font-dm text-xs">{selected.email}</p>
              </div>
