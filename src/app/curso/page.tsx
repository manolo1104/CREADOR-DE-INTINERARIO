...
export default async function CursoPage() {
  ...
  return (
    <main className="bg-tinta text-hielo md:pt-[52px]">
      ...
      {/* ══ ANTES / DESPUÉS ══ */}
      {/* Bloque BLANCO. Rompe el negro a media página, justo donde el ojo
          se empieza a cansar, y el contraste hace que la comparación se lea
      ...
      <section className="bg-blanco px-5 py-16 text-tinta md:py-24">
        <div className="mx-auto max-w-4xl">
          ...
          <div className="mt-10">
            <div className="hidden grid-cols-2 gap-8 pb-3 font-mono text-[11px] font-medium uppercase tracking-[0.16em] text-grafito sm:grid">
              <span>Hoy</span>
      ...
      <section className="bg-blanco px-5 py-16 text-tinta md:py-24">
        <div className="curso-faq mx-auto max-w-3xl">
          ...
          <div className="mt-8">
            {FAQS.map((f) => (
              <details key={f.p} className="group border-t border-linea-clara py-1 last-of-type:border-b">
                <summary className="flex cursor-pointer list-none items-center justify-between gap-4 py-5 font-dm text-lg font-medium text-tinta [&::-webkit-details-marker]:hidden">
                  {f.p}
                  <svg aria-hidden="true" className="h-5 w-5 shrink-0 text-azul-hondo transition-transform duration-200 ease-out group-open:rotate-45" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M12 5v14M5 12h14" /></svg>
                </summary>
                <p className="max-w-[60ch] pb-6 font-dm text-lg leading-relaxed text-grafito">{f.r}</p>
              </details>
            ))}
          </div>
          <div className="mt-10 border border-linea-clara bg-[#F5F8FC] p-7 text-center">
            <p className="font-dm text-lg text-grafito">¿Tienes otra duda? Pregúntame directo, te contesto yo.</p>
            <a
              href={WHATSAPP_CURSO}
              target="_blank"
              rel="noopener noreferrer"
              className="mt-4 inline-flex items-center gap-2.5 bg-[#25d366] px-8 py-4 font-dm text-base font-semibold uppercase tracking-[1.5px] text-tinta transition-transform duration-150 ease-out active:scale-[0.98]"
            >
